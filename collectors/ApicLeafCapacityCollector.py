...
class ApicLeafCapacityCollector(Collector):
    ...
    def describe(self):
        yield GaugeMetricFamily('network_apic_leaf_capacity', 'ACI Leaf capacity')
                ...
                if not self.gen1_leaves.get(leaf_id, False):
                    for data_object in leaf_data:
                        if 'eqptcapacityL3TotalUsageCap5min' in data_object:
                            ...
                            g_leaf_cap_tcam.add_metric(labels=[leaf_id, 'max', 'l3'], value=l3_max)
                            ...
                            g_leaf_cap_tcam.add_metric(labels=[leaf_id, 'local', 'l3'], value=l3_local)
                            ...
                            g_leaf_cap_tcam.add_metric(labels=[leaf_id, 'remote', 'l3'], value=l3_remote)
                            ...
                            g_leaf_cap_tcam.add_metric(labels=[leaf_id, 'max', 'l2'], value=l2_max)
                            ...
                            g_leaf_cap_tcam.add_metric(labels=[leaf_id, 'local', 'l2'], value=l2_local)
                            ...
                            g_leaf_cap_tcam.add_metric(labels=[leaf_id, 'remote', 'l2'], value=l2_remote)
                else:
    ...
    def _get_gen1_leaves(self, host: str) -> Dict:
        ...
        result = self.query_host(
            host, '/api/node/class/fabricNode.json' + '?query-target-filter=and(' +
            'or(wcard(fabricNode.model,"PQ"),wcard(fabricNode.model,"PX"))' + ',eq(fabricNode.role,"leaf"))')
        gen1_leaves = {}
        for leaf in result['imdata']:
            leaf_id = leaf['fabricNode']['attributes']['dn'].split('/')[2]
